...
(() => {
    ...
    const { request } = createApi(window.DocFlow);
    const { render, renderActivity } = createView({ $, escape, date, getRows: () => rows, filterBindings: window.WatchFolderViewModels.filterBindings });
    async function load() {
    ...
    async function activity() {
        ...
        renderActivity(payload, activityId, activityOffset);
    }
