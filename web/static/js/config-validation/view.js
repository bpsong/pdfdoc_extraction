/** Rendering and presentation for config-validation. */
export function createView(deps) {
    const { state, docFlow } = deps;
    const SECRET_KEYS = ['api_key', 'apikey', 'password', 'password_hash', 'secret', 'secret_key', 'token', 'access_token', 'refresh_token', 'authorization'];
    ...
    return { escapeHtml, secretKey, redactSecrets, normalizeFinding, summaryFromResult, badgeClass, renderValidationSummary, renderFindings, renderRawJson, render };
}
