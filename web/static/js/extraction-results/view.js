/** Rendering and presentation for extraction-results. */
export function createView(deps) {
    const { documentId, title, statusLabel, previewBody, fileList, tableBody, jobLabel, providerLabel, rawPayload, reviewLink, reviewSummary, previousButton, nextButton, docFlow, pdfRuntime } = deps;
    let pdfViewer = null;
    ...
    function statusBadge(status) {
        return `<span class="badge ${docFlow.statusBadgeClass(status, "unknown")} badge-sm">${escapeHtml(docFlow.statusLabel(status, "unknown"))}</span>`;
    }
    ...
    return { escapeHtml, titleCase, formatValue, statusBadge, confidenceBadge, nestedConfidenceSummary, renderPreview, renderFiles, effectiveReviewStatus, renderFields, wireSiblingButtons, renderPayload };
}
