/** Rendering and presentation for reports. */
export function createView(deps) {
    const { batchDetailTitle, batchDetailSubtitle, batchDetailBody, batchDetailProcessingLink, docFlow } = deps;
    ...
    function statusBadge(status) {
        return `<span class="badge badge-sm ${docFlow.statusBadgeClass(status, "unknown")}">${escapeHtml(docFlow.statusLabel(status, "unknown"))}</span>`;
    }
    ...
    function renderRecentBatches(batches) {
        ...
        body.innerHTML = batches.map((batch) => `
            <tr class="reports-batch-row" role="button" tabindex="0" data-batch-id="${escapeHtml(batch.id)}" aria-label="View batch ${escapeHtml(batch.id)} workflow details">
                <td class="text-xs">${escapeHtml(docFlow.formatDateTime(batch.created_at))}</td>
    ...
    return { escapeHtml, titleCase, statusBadge, workflowState, formatDuration, formatDateTime, safeJsonBlock, renderMetric, renderSimpleRows, renderSourceRows, renderRecentBatches, renderBatchSummary, renderTaskRows, renderDocumentDetails, renderBatchDetails, render };
}
