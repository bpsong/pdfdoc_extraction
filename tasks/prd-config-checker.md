...
- **Module-aware extraction prerequisites**: For modules under standard_step.extraction.* (and custom equivalents), require configs to declare `api_key` before runtime and emit actionable errors when it is missing or empty. For LlamaCloud Extract v2, `configuration_id` is optional because tasks can build an inline schema from `fields`; if present, it must be a non-empty string. This enforcement also applies when extraction tasks are inferred from their fields map even if module metadata is omitted.
...
- **Storage metadata readiness signal**: Emit a warning when v2 storage tasks cannot locate extraction.fields metadata from a configured extraction task so teams can reconcile alias gaps before deployment.
