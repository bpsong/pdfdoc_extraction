...
| `param-glm-invalid-prompt-style` | Error | Prompt construction mode is unsupported | Use `detailed`, `compact`, or `verbatim` |
| `param-glm-missing-verbatim-instructions` | Error | Verbatim prompt mode has no complete prompt text | Supply non-empty `document_instructions` |
| `param-glm-invalid-schema-order` | Error | A GLM schema position is not a positive integer | Set `schema_order` to a positive integer |
| `param-glm-duplicate-schema-order` | Error | Two sibling fields use the same GLM schema position | Give each sibling field a unique `schema_order` |
